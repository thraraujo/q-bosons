...
\subsubsection{Dyson gases}

It is also interesting to consider the particular case when \(M\to
\infty\), but with a finite number of particles \(N\). The
partititions \(\lambda \in [N, \infty]\) satisfy the condition
...
written as the matrix integral
\begin{equation}
  S_N(\bm{t}, \bm{t}') =
...
\end{equation}
Then, the phase model is equivalent to an ensemble of \(N\)
2D Coulomb particles on a circle. 
...
%
%\input{./sections/schurpol.tex}
%
%\input{./sections/transfer.tex}
%
%\input{./sections/t-fermions.tex}
%
%\input{./sections/top-strings.tex}
